...
By default, the standalone server hosts a standard Godot ENet server on port **25567** (RPC/UDP) and asset streaming on port **25568** (HTTP/TCP, always port + 1). You can also change the port at runtime in the server console with `/port <number>`.

If your team is connecting over the internet, you must ensure that **Port 25567 (UDP)** and **Port 25568 (TCP)** are open/forwarded on your firewall or router.
